...
class berita_model extends CI_Model {
	...
	{
		$this->db->select('SAS_BERITA_TITLE, SAS_BERITA_CONTENT, SAS_BERITA_TAG, SAS_PUBLISH_FLAG');
		$this->db->where('SAS_ACTIVE_FLAG ', 1);
		$this->db->where('SAS_BERITA_ID', $this->uri->segment(3));
		$this->db->where('SAS_ADDED_BY_NAME', $this->session->userdata('SAS_USER')['SAS_USER_NAME']);
		$this->db->where('SAS_ADDED_BY', $this->session->userdata('SAS_USER')['SAS_USERNAME']);		
		$query = $this->db->get('sas_berita');
		if ($query->num_rows() != 0)
		{
			return $query->result_array();
